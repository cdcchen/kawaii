<?php
/**
 * @link http://www.kawaiiframework.com/
 * @copyright Copyright (c) 2008 Kawaii Software LLC
 * @license http://www.kawaiiframework.com/license/
 */

namespace kawaii\helpers;
...
class BaseUrl
{
     ...
     *
     * This method will use [[\kawaii\web\UrlManager]] to create a URL.
     *
     ...
     *
     * - If the route is an empty string, the current [[\kawaii\web\Controller::route|route]] will be used;
     * - If the route contains no slashes at all (e.g. `index`), it is considered to be an action ID
     *   of the current controller and will be prepended with [[\kawaii\web\Controller::uniqueId]];
     * - If the route has no leading slash (e.g. `site/index`), it is considered to be a route relative
     *   to the current module and will be prepended with the module's [[\kawaii\base\Module::uniqueId|uniqueId]].
     *
     ...
     * - `true`: returning an absolute base URL whose scheme is the same as that in [[\kawaii\web\UrlManager::hostInfo]].
    ...
    {
        $route = Kawaii::getAlias((string) $route);
        ...
        // relative route
        if (Kawaii::$app->controller === null) {
            throw new InvalidParamException("Unable to resolve the relative route: $route. No active controller is available.");
        }

        if (strpos($route, '/') === false) {
            // empty or an action ID
            return $route === '' ? Kawaii::$app->controller->getRoute() : Kawaii::$app->controller->getUniqueId() . '/' . $route;
        } else {
            // relative to module
            return ltrim(Kawaii::$app->controller->module->getUniqueId() . '/' . $route, '/');
        }
    }
     ...
     * [[\kawaii\web\UrlManager::hostInfo]]) will be returned. If `$url` is already an absolute URL, its scheme
    ...
    public static function to($url = '', $scheme = false)
    {
        ...
        $url = Kawaii::getAlias($url);
        if ($url === '') {
            $url = Kawaii::$app->getRequest()->getUrl();
        }
     ...
     * later by [[previous()]]. If not set, it will use [[\kawaii\web\User::returnUrlParam]].
     * @see previous()
     */
    public static function remember($url = '', $name = null)
    {
        ...
        if ($name === null) {
            Kawaii::$app->getUser()->setReturnUrl($url);
        } else {
            Kawaii::$app->getSession()->set($name, $url);
        }
    }
     ...
     * If not set, it will use [[\kawaii\web\User::returnUrlParam]].
    ...
    {
        if ($name === null) {
            return Kawaii::$app->getUser()->getReturnUrl();
        } else {
            return Kawaii::$app->getSession()->get($name);
        }
    }

    /**
     * Returns the canonical URL of the currently requested page.
     * The canonical URL is constructed using the current controller's [[\kawaii\web\Controller::route]] and
     * [[\kawaii\web\Controller::actionParams]]. You may use the following code in the layout view to add a link tag
     * about canonical URL:
     *
     * ```php
    ...
    {
        $params = Kawaii::$app->controller->actionParams;
        $params[0] = Kawaii::$app->controller->getRoute();
    ...
    {
        $currentParams = Kawaii::$app->getRequest()->getQueryParams();
        $currentParams[0] = '/' . Kawaii::$app->controller->getRoute();
    ...
    {
        return static::$urlManager ?: Kawaii::$app->getUrlManager();
    }
}
