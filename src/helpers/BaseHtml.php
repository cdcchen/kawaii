...
class BaseHtml
{
     ...
     * The [[\kawaii\base\Application::charset|application charset]] will be used for encoding.
    ...
    {
        return htmlspecialchars($content, ENT_QUOTES | ENT_SUBSTITUTE, Kawaii::$app ? Kawaii::$app->charset : 'UTF-8', $doubleEncode);
    }
     ...
     * See [[\kawaii\web\Request::methodParam]] for more details.
     * @param array $options the tag options in terms of name-value pairs. These will be rendered as
     * the attributes of the resulting tag. The values will be HTML-encoded using [[encode()]].
     * If a value is null, the corresponding attribute will not be rendered.
     * See [[renderTagAttributes()]] for details on how attributes are being rendered.
     ...
     * [[\kawaii\helpers\ArrayHelper::map()]].
     *
    ...
    {
        $header = isset($options['header']) ? $options['header'] : '<p>' . Kawaii::t('kawaii', 'Please fix the following errors:') . '</p>';
     ...
     * the `maxlength` option will take the value of [[\kawaii\validators\StringValidator::max]].
     * @param Model $model the model object
     ...
     *   by a string validator, the `maxlength` option will take the value of [[\kawaii\validators\StringValidator::max]].
     ...
     * if `'data' => ['id' => 1, 'name' => 'kawaii']`, then this will be rendered:
     * `data-id="1" data-name="kawaii"`.
     * Additionally `'data' => ['params' => ['id' => 1, 'name' => 'kawaii'], 'status' => 'ok']` will be rendered as:
     * `data-params='{"id":1,"name":"kawaii"}' data-status="ok"`.
     *
    ...
    public static function getAttributeValue($model, $attribute)
    {
        ...
        // https://github.com/kawaiisoft/kawaii2/issues/1457
        if (is_array($value)) {
