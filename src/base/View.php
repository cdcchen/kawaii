...
class View extends Object
{
    ...
    public function init()
    {
        ...
        if (is_array($this->theme)) {
            if (!isset($this->theme['class'])) {
                $this->theme['class'] = 'kawaii\base\Theme';
            }
            $this->theme = Kawaii::createObject($this->theme);
    ...
    protected function findViewFile($view, $context = null)
    {
        if (strncmp($view, '@', 1) === 0) {
            // e.g. "@app/views/main"
            $file = Kawaii::getAlias($view);
        } elseif (strncmp($view, '//', 2) === 0) {
            // e.g. "//layouts/main"
            $file = Kawaii::$app->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
        } elseif (strncmp($view, '/', 1) === 0) {
            // e.g. "/site/index"
            if (Kawaii::$app->controller !== null) {
                $file = Kawaii::$app->controller->module->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
            } else {
    ...
    public function renderFile($viewFile, $params = [], $context = null)
    {
        ...
        if ($this->theme !== null) {
            // todo theme apply to viewFile
//            $viewFile = $this->theme->applyTo($viewFile);
        }
        if (is_file($viewFile)) {
            // @todo localize viewfile
        ...
        if ($this->beforeRender($viewFile, $params)) {
            ...
            if (isset($this->renderers[$ext])) {
                if (is_array($this->renderers[$ext]) || is_string($this->renderers[$ext])) {
                    $this->renderers[$ext] = Kawaii::createObject($this->renderers[$ext]);
                }
