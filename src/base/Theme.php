...
class Theme extends Object
{
    ...
    public function applyTo(string $path): string
    {
        ...
        if (empty($pathMap)) {
            ...
            $pathMap = [Kawaii::$app->getBasePath() => [$basePath]];
        }

        $path = FileHelper::normalizePath($path);

        foreach ($pathMap as $from => $tos) {
            $from = FileHelper::normalizePath(Kawaii::getAlias($from)) . DIRECTORY_SEPARATOR;
            if (strpos($path, $from) === 0) {
                $n = strlen($from);
                foreach ((array)$tos as $to) {
                    $to = FileHelper::normalizePath(Kawaii::getAlias($to)) . DIRECTORY_SEPARATOR;
